...
def sampling_options(reasoning_effort):
    return {
        "temperature": 1.0,
        "top_p": 0.95,
...
provider["vllm"]["models"] = {
    "glm-5.3-flash": {
        ...
        "options": sampling_options("max"),
    }
}
config["model"] = "vllm/glm-5.3-flash"

profiles = {"general": "max", "scout": "high", "explore": "low"}
agent_defaults = {
    "general": {
    ...
    cleaned.update(
        {
            "temperature": 1.0,
            "top_p": 0.95,
            "options": sampling_options(effort),
        }
    )
    agents[name] = cleaned
