...
usage() {
    ...
    echo "  --glm53-gb10                 : Build the qualified GLM-5.3 GB10 native FP8 DS-MLA image"
...
# The GLM-5.3 GB10 profile uses a digest-pinned official base plus the native
# FP8 NoPE patch chain. Overrides would silently turn it into a different,
...
if [ "$NO_BUILD" = false ]; then
    if [ "$QWEN38_FLASH_NEXT" = true ]; then
        ...
        GLM53_CMD=("docker" "build" "-f" "Dockerfile.glm53-fp8-ds-mla" "-t" "$IMAGE_TAG")
        if [ "$FULL_LOG" = true ]; then
        ...
        echo "Building qualified GLM-5.3 native FP8 DS-MLA image with command: ${GLM53_CMD[*]}"
        RUNNER_START=$(date +%s)
        "${GLM53_CMD[@]}"
        RUNNER_END=$(date +%s)
        RUNNER_BUILD_TIME=$((RUNNER_END - RUNNER_START))
